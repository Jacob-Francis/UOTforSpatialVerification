...
def test_marginals_energy_cpu(n, m, t_n):
    if t_n == "torch":
...
def test_marginals_energy_pykeops_flat(n, m, t_n):
    if t_n == "torch":
        X = torch.cartesian_prod(torch.linspace(0, 1, n), torch.linspace(0, 1, n))
        Y = torch.cartesian_prod(torch.linspace(0, 1, m), torch.linspace(0, 1, m))
    else:
...
def test_marginals_tensor_tuple(n1, n2, m1, m2, keops):
    # tuple input
...
def test_marginals_tensor(n, m, keops):
    X1, X2 = np.meshgrid(
        torch.linspace(0, 1, n), torch.linspace(0, 1, n), indexing="ij"
    )
    Y1, Y2 = np.meshgrid(
